...
def build_data_provider(local_batch_size, examples_per_user, drop_last: bool = False):
    ...
    Save_KL_Result("FL_non_IID_ILP_MNIST(CNN)", KL_of_each_client, avg_KL)
...
def main(
    ...
    Save_Accuracy_of_each_epoch(1, "FL_non_IID_ILP_MNIST(CNN)", accuracy_of_each_epoch,best_accuracy_of_each_epoch)
    global total_execution_time
    client_num=num_of_original_client
    CreateResultData("FL_non_IID_ILP_MNIST(CNN)", "MNIST", "CNN", "non-IID -> IID", client_num, int(trainer_config.epochs), eval_score['Accuracy'], total_execution_time)
   

@hydra.main(config_path="configs", config_name="MNIST_config" , version_base="1.2")
def run(cfg: DictConfig) -> None:
    print('-------------------FL_non_IID_ILP_MNIST(CNN)-------------------')
    #print(cfg)
