...
def FKLD(NCD,NMD,CD,MD_label,MD_label_len,comp):
    label = 10
    ...
    for i in range(NCD):
        for j in range(NMD):
            MD_CD_total_label[(i,j)] = MD_label[i] + CD.label[j]
    ...
    for i in range(NCD):
        for j in range(NMD):   
            MD_CD_total_label_P[(i,j)] = MD_CD_total_label[(i,j)] /(MD_label_len[i]+CD.label_len[j])
    ...
    for i in range(NCD):
        for j in range(NMD):
            if(i != j):
                ...
                KL[i][j] = np.NaN
    """
    #print("not_comp_in:",not_comp)       
    #print("KL:",KL)
    #print("not_com:",not_comp)
    return(KL)
    #np.savetxt("data3.csv", KL,fmt='%1.4f',delimiter = ",")
def FKLD_noself(NCD,NMD,CD,MD_label,MD_label_len,comp):
    label = 10
    ### MD
    MD_CD = np.array([NCD,NMD])
    MD_CD_total_label = dict()
