...
def ILP_method(train_dataset, num_of_label, client_data_index, original_client_num, used_client_num, head_num,
               ILP_Max_value):
    total_time = 0
    # client information
    num_of_client = used_client_num
    ...
    while (unassigned_member_index):
        ...
        for i in range(len(selected_head_group)):
            ...
            for j in range(len(head_index_group[selected_head_index])):
                for t in range(label_num):
                    P_i_list[t] = P_i_list[t] + nums_of_labels_of_each_client[head_index_group[selected_head_index][j]][
                        t]
            ...
            for j in range(len(Combination_of_C_M)):
                ...
                KL = KL_of_combination(label_num, size_of_combination_head + size_of_combination_member, P_i_list,
                                       P_j_list, distribution_Q)
        ...
        for i in range(C_h):
            ...
            for j in range(C_M_Combination_num):
                if assigned_member_group[i][j] == 1:
                    print("x(", i, ",", j, ")=1", " | head :", head_index_group[selected_head_group_index][0],
                          " <= member", Combination_of_C_M[j])
...
def main():
    ...
    heuristic_index_of_head_group, time = heuristic_method(train_dataset, num_of_CIFAR10_label, dict_users, client_num,
    ...
    ##########ILP method##########
    print("##########ILP method##########")
    ILP_index_of_head_group, ILP_time = ILP_method(train_dataset, num_of_CIFAR10_label, dict_users, client_num,
                                                   num_of_used_client, num_of_head_client, Max_value_of_ILP)
